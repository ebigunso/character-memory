...
`RetrievalContext` carries a `Scene` and an optional topic; its time defaults to now. Participant keys and exact names cue notions, while participant descriptions and setting words search memory content. A setting key or custom value recalls current beliefs grounded in experiences with that context; beliefs with several sources can return in any of their recorded places or named contexts. The result returns the present scene, its reference resolutions, and each admitted memory's recorded source scenes even without a trace. Forgotten sources are explicit; forgotten scenes follow `include_suppressed`. Scene differences never exclude a memory or determine who may hear it.
Each scene reference reports `last_interactions`, keyed by every notion it resolves, including all candidates of an ambiguous name. A value gives the last episode id, its recorded scene time, and the whole seconds since it relative to the retrieval scene time; `null` means never met at or before that time. Descriptions and unknown references report an empty map. These facts are available without a trace and independently of pack caps; suppression follows `include_suppressed`.
...
Candidate, graph root and section selection give each cue kind its own queue. Content cues follow their own search order; explicit cues keep their selector order. At section selection, explicit roots and what they brought come first, then direct word matches, then memories that only inherited the kind from other roots in section score order. A keyed participant's occasions keep their recorded scene-time order, latest first. Each turn takes that queue's head. An already selected memory uses no extra slot and credits another kind only when it reaches that kind's head. Selection first serves the configured numeric floors, then shares spare slots in the same turns among every kind present, including those with a zero floor. Scores from different searches do not decide the remaining admissions; the pack keeps its final score order.
...
A participant present in most experiences brings fewer occasions to mind, with a minimum of the latest one. Occasions are selected by recorded `Scene.time`; a participant named by key or known name brings only occasions at or before the retrieval scene time, with one shared episode budget across direct episode links and observation links, and at most one observation per selected episode on the observation route. Frequency counts distinct episodes, so several observations or participants in one episode do not inflate its weight. This rule applies to every notion, including whichever one the application regards as the character; beliefs about a familiar participant keep their existing retrieval rule.

Current beliefs about a participant named by key or known name are selected by salience, then creation recency, then id, within the existing graph caps. Their own subject lists determine aboutness, regardless of who was present when they were formed. Superseded beliefs are omitted unless `include_superseded` is set, in which case current beliefs take priority in neighbour selection. When several named participants bring state into a section, their memories are served in scene order, one per participant per round, preserving each participant's final relevance ranking; a memory about several participants counts for each. Topic-only retrieval and content-derived entity expansion retain their selection rules.
...
For the state route, an incoming `Resolves` or `FulfillsCommitment` link also ends current state. Named-subject state, activity thread members, and setting/custom-key state omit these memories with `ResolvedOmitted`. Topic and other recall remain eligible. Each admitted interpreted memory carries `resolved_by`, a sorted list of resolving memory ids (empty when unresolved), even without a trace. Recalled resolved open loops and fulfilled commitments go into `derived_memories`, using its section limit instead of taking an `open_loops` or `commitments` slot; their scores, stored subtypes, and resolver evidence are preserved. Suppressing a resolver does not reopen its target. Resolution leaves the target's content and retention unchanged.
