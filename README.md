...
Graph root selection shares the remaining slots in successive rounds among cue kinds with a nonzero floor, preserving each kind's internal order, so a large activity leaves room for several topic memories. Candidate and section selection return remaining slots to score order.
