...
A participant present in most experiences brings fewer episodes and observations to mind. Frequency counts distinct episodes, so several observations or participants in one episode do not inflate its weight. This rule applies to every notion, including whichever one the application regards as the character. Beliefs about a participant the scene names are not reduced by familiarity.
...
A scene setting key or custom value can recall current beliefs formed in that context without a topic or participant. Beliefs grounded in several experiences can return in any of their recorded places or named contexts.
