...
A participant present in most experiences brings fewer episodes and observations to mind. Frequency counts distinct episodes, so several observations or participants in one episode do not inflate its weight. This rule applies to every notion, including whichever one the application regards as the character.

Current beliefs about a participant named by key or known name are selected by salience, then creation recency, then id, within the existing graph caps. Their own subject lists determine aboutness, regardless of who was present when they were formed. Superseded beliefs do not consume this state selection. When several named participants bring state into a section, their memories are served in scene order, one per participant per round; a memory about several participants counts for each. Topic-only retrieval and content-derived entity expansion retain their selection rules.
