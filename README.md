...
A scene time reads back with the instant, fractional precision and offset supplied by the application. Supported offsets are whole minutes from -23:59 through +23:59, so returned scenes also round-trip through RFC 3339 JSON losslessly. Plan validation, commit and retrieval reject offsets with a seconds part with a typed error carrying the offset in seconds. Scene construction and preparation remain infallible data assembly. Anniversaries match the local calendar day of the offset each experience was written with against the retrieval scene's local day; library-generated scene times use UTC. Replaying the same ID and instant with a different offset is a collision, even when the local day is unchanged.
