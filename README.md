...
Candidate, graph root and section selection give each cue kind its own queue. Content cues follow their own search order; explicit cues keep their selector order. At section selection, memories that only inherited a kind through expansion follow its direct matches, in section score order. Each turn takes that queue's head. An already selected memory uses no extra slot and credits another kind only when it reaches that kind's head. Selection first serves the configured numeric floors. Roots then share spare slots in the same turns among kinds with a nonzero floor; candidate and section selection return remaining slots to score order.
