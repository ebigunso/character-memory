...
4. **Use one rank order.** Roots sort by score, salience, timestamp presence, newest timestamp, road row, position in that road's list, then ID. Salience and timestamp enter this key only at score zero when a time or keyed-place road reached the memory. Search-only roots, including descriptions, carry neither. After reservations, only expanding roads share spare root turns, and only when at least two kinds have them. Other stages fill ranked room. Section ties use memory time, then type and ID; candidate-merge ties use type and ID. Scopes of one given kind retain their own turns.
