...
Retain the measured retrieval defaults: smoothing and fanout shaping at one, the relation/object fanout ranges of zero to twenty for beliefs about a notion, one to five for participant experiences and zero to fifteen for thread memories, and candidate limits of 48 vector candidates and twelve graph roots. Include one reserved slot each for participant, place, activity and topic as the measured cue-floor defaults. A floor reserves room per given kind, not per person or place; unused room remains available to the other memories. Topic-only retrieval keeps its selection.
...
Beliefs about a notion the scene names are chosen outside entity-root selectivity, within the existing hard bounds. Naming a notion is a direct cue to what is known about it, not an inference from its rarity. The retained inverse-frequency rule does not suppress that state; other entity-root expansion keeps its measured scope, without generalizing the rule to non-entity roots.

## Why

The defaults are retained on measured evidence: one reserved slot per given cue kind prevents starvation by another, while more reserved scene room admits weaker occasions at the conversation's expense. Participant reach follows the share of experiences the notion takes part in, whereas beliefs about a notion the scene names are chosen outside entity-root selectivity because a named root is not inferred.
...
- Pin the library and evaluation inputs, vary one default at a time, repeat each configuration and compare returned identities, order, context size and native cue evidence against the topic-alone and description-removed controls.
- Exercise participant and place pressure separately and together, including reworded descriptions, unfamiliar descriptions, and IDs opposing occasion time; report which cue kinds and pressures ran, rather than treating an unchanged inactive floor as proof of protection.
...
A measured similarity bound for descriptions or a relevance bound for the topic changes which memories compete; the phase re-baseline changes retrieval routes or pack admission; a representative corpus replaces the synthetic workload; or a different statistics regime invalidates the scored-decision calibration. Repeat the affected calibration before changing the values. The scene-overlap measurement uses deterministic synthetic embeddings, so production embedding behavior requires separate evidence.
...
- The public companion evaluation repository, `CharacterMemoryEvals`, retains the scene reminder calibration in `.agent-work/worker2/task9/after-the-fix.md`, with its artifact manifest and raw measurements.
...
- The [cues and floors plan](../../coding-agent/plans/active/v0-2-cues-and-floors-plan.md) and [state and scope plan](../../coding-agent/plans/completed/v0-2-state-and-scope-plan.md) record the experience-share and named-notion-state rulings.
