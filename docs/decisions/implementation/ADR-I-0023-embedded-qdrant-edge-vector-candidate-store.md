...
Searches and index builds never run on an async executor thread: the adapter invokes the engine from a blocking worker, or through the engine's own worker pool where its API is asynchronous, and the in-phase benchmark records executor responsiveness while a scan or build is in progress.
