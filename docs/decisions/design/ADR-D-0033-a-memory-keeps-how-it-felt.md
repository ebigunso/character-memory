...
# ADR-D-0033: A memory keeps how it felt: feeling is part of the episode and is carried in words
...
An episode keeps how it felt. The gist of an episode is the character's own account (ADR-D-0020) and says, in words, how the episode landed for it, where the trace shows it: in what the character itself said and did, in a thought or a note it made at the time. Where nothing shows how it felt, the gist does not supply a feeling. A later change of heart needs no rule of its own. It supersedes the account it reconsiders, restated as any changed understanding is, "I was hurt then; I see it differently now", so the current memory says both, the earlier account stays in the chain behind it and can be traced when it is wanted, and it never surfaces alone as though nothing had changed.
...
How something felt is what lets a character approach a sore subject with care and carry yesterday into today, which is most of what makes continuity feel like someone's. Holding a change of heart the way every other change of understanding is held, by supersession with a restatement, gives the character its arc without a second mechanism: it can say how it felt then and how it sees things now, and the record keeps both.
...
- Exempting feeling from supersession, so that a reappraisal stands beside the earlier account and both stay current: rejected because it is an extra rule that buys nothing the supersession chain does not already keep, and it lets the old account surface by itself, as though the character had never thought again.
...
Invariant: an episode's gist says how it felt to the character where the character's own trace shows it, and invents no feeling where it does not; a change of heart is ordinary supersession with a restatement that says both how it felt and how it looks now; an impression is an observation attributed to the character as its own perception, grounded like any other, and supports a conclusion about a person only through a pattern; no memory carries a numeric measure of emotion, and feeling has no recall route of its own.
...
- After the character reconsiders an old hurt, recall returns one current memory that says both how it felt then and how it sees it now, the earlier account is reachable through the chain, and it does not surface alone (the catalog's F23).
