...
# ADR-D-0022: Recall is activation by the cues of the present scene, with a route per cue kind, and purpose is never a supplied cue

## Context and Problem Statement

Retrieval seeds from one route, the content of the current query, and expands through the graph from there. Most of what a person carries into a moment has no topic yet: who is here, what was last said with them, what is owed, what is in progress, what fell due, what happened this morning. A walk through an ordinary day across companions, workers, characters, and independent entities found that recall is driven by several kinds of cue, the people present, the place, the time and date, the activity in progress, a stored intention's trigger, and the topic, and that any single-route pipeline leaves most of the day unreachable except by faking a query. The fork is whether recall stays one route with a ranking signal for time, or becomes activation by whatever cues the scene supplies, and whether the character's purpose is one of those cues.
...
Purpose is never a supplied cue. What the character is trying to do surfaces from memory, as an open loop, a commitment, a thread in progress, or a character signal, and once surfaced it re-cues one bounded hop. The retrieval input carries no goal, intent, or purpose field.

## Why

A character that arrives at a moment carrying only what the topic retrieves is a new instance with notes, which is the failure the library exists to remove. A purpose handed in from outside is a persona assigned for one turn; a purpose that comes up from what the character remembers is accumulated character, and letting memory shape behavior means the why must come from memory.

## Rejected Alternatives

- One content route with time as a ranking signal: rejected outright; every situation without a topic stays unreachable, and a ranking signal cannot admit what was never a candidate.
- A separate current-state call beside retrieval: rejected because it is the same activation with an empty content route, and two surfaces would drift; reopen only if measurement shows the two need different budgets that one retrieval cannot express.
- A goal or intent field on the retrieval input: rejected outright for the reason in Why; a dispatched task's purpose arrives in the interaction as content and as an open loop, and reaches recall that way.
...
Invariant: retrieval takes the present scene and activates candidates through a route per cue kind with a floor per route; the retrieval input carries no purpose; currency never selects items; the re-cue from surfaced state is bounded to one hop.
