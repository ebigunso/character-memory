...
```text
gist episodes, each with its scene, the identifiers of the entries it consolidated, and every source pointer those entries supplied; the gist is the character's own account and says how the episode felt to it, which is never revised afterward (ADR-D-0033); an episode may be marked unfinished
