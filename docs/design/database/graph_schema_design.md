...
`Scene.time` preserves the supplied instant and offset. RDF stores the instant in UTC as `sceneTime` and the offset in seconds east of UTC as the required `sceneOffsetSeconds` literal. Hydration restores that offset without changing the instant or fractional precision; replay equality compares both instant and offset. The `sceneLocalYear` and `sceneMonthDay` literals are an anniversary index derived from `scene.time`'s local date when writing the episode. They are not separate domain content: a UTC calendar day would invent the experienced day when the supplied offset crosses midnight. Time queries use the UTC instant, and the offset is read only when hydrating a scene. Stores are regenerated for this schema; there is no compatibility read for episodes without an offset.
