...
Source lookup, thread lookup, name lookup, scope-key lookup and bounded expansion query the named graphs they need. [Object hydration (`hydrate_objects_by_refs_from_store`)](../../../src/adapters/oxigraph/shared.rs) reads the named graphs of the selected object references, including their owned assertion subjects, rather than every stored quad. Expansion is bounded by depth, object and relation scope, lifecycle policy and fanout caps.
