...
The episode owns the scene. `sceneTime` stores its timestamp losslessly; `sceneParticipants` and `sceneCustomValues` store JSON literals, preserving participant order, duplicates, spelling and custom string values. `settingKey` is a directly queryable literal, separate from `settingWords`. Keys identify application contexts, not notions; sessions may be recorded in custom values. Each participant supplies at least one of key, name or description; an all-absent participant is rejected. Participant keys must identify existing or same-plan notions. Names, descriptions and setting words are preserved as given and also indexed with the episode summary in its one embedding; they create no notions or links. Setting words follow a `Setting:` label and each participant name and description follows a `With:` label in authored order, including when a key is also present. The embedding builder folds whitespace and omits blank word values; the stored scene is unchanged. Keys and custom values are never embedded. Without scene words the episode embedding text is unchanged. Writes reject an activity.
