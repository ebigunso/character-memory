...
Pack expansion needs multiple occasions and exclusion evidence, so it retains a separate occasion read. The [participant limiter (`graph_expansion.rs:1145`)](../../../src/policy/graph_expansion.rs#L1145) shares one episode budget across direct episode and observation routes, with at most one observation per selected episode. Frequency counts distinct episodes, and selection prefers recorded scene time, latest first. Only participants recognized by key or name apply the retrieval scene-time cutoff here; beliefs and other retrieval paths have no new as-of rule.
