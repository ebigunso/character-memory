...
Both adapters answer through the shared tie-closure loop and the canonical constructor; before this phase the service adapter returned the truncated set silently at its bound, and the port's bare list type could not say whether even the returned prefix's cutoff was closed.
...
- The in-process build of the service backend (Qdrant Edge, pinned exactly at 0.8.0, beta) is the embedded engine: measured on the spike as lifecycle-and-reopen identical, exactness controllable by the indexing threshold, and five-of-five service parity at score delta 0.0. The 2026-09-04 in-phase release probe on Windows x86-64 measured 126,464 bytes without the engine and 24,465,920 bytes with it, a 24,339,456-byte delta; `llvm-strip` changed neither PE image, so the stripped delta was also 24,339,456 bytes. The direct probe's normal dependency tree grew by 746 lines. Qdrant Edge 0.8.0 publishes no Cargo feature flags, so feature trimming cannot reduce this version's graph. The engine requires Rust 1.97 or later.
- The 2026-09-04 ignored benchmark at the configured 1,536 dimensions measured exhaustive scans at 3 ms for 100 records, 11 ms for 1,000 records, and 48 ms for 5,000 records; writing and flushing those corpora took 31 ms, 202 ms, and 914 ms. An indexed 1,000-record control overlapped the exhaustive top 48 by 48 of 48 and built in 699 ms. Executor heartbeats advanced during construction, every write burst and scan, the build, close, a 1,042 ms lock-backoff wait, and the following 164 ms reopen, demonstrating that the dedicated owner kept engine work off the async executor on this Windows x86-64 run. These figures are dated local guidance, not portable latency guarantees.
...
No public facade change beyond the telemetry field, the published maximum-surfaces-per-object-kind policy value, and consuming awaitable close for deterministic store release; no retrieval behaviour change in service mode for non-empty scopes and non-degenerate queries (an empty object-type scope now selects zero instead of searching unfiltered, and an empty configured scope is rejected at the boundary, both intended).
```
