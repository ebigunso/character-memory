...
Verdict mapping: exhaustive when the shard is unindexed and the scan returned fewer rows than the fetch limit (the whole scoped population came back), boundary tie closed or open from the loop otherwise, not requested at limit zero.
