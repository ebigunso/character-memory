...
- 2026-09-21 Decision: Task_1 is built from its baseline, and two things leave it.
  - Trigger / new insight: run at the parent commit, every ordinary case already passed: the existing expansion through beliefs about a notion already brings what the character holds about a person, excludes what merely happened beside them and what was superseded, and handles an ambiguous name. Nothing was built for those. The hard cases failed: a person with 200 current beliefs brought NONE, because the measured inverse-frequency rule on beliefs gives a well-known notion a cap of zero; and where more beliefs exist than fit, they were taken in id order, so clearly salient ones were lost.
  - Plan delta (what changed): for a root the scene names, beliefs about it are chosen by salience, then recency, then id, up to the existing maximum, outside the inverse-frequency cap; entity roots reached from content hits keep the measured rule. Scopes share section room in rounds. No separate selector and no state roots: the fix is where neighbours are chosen. The sentence that a surfaced open loop brings the experiences it rests on leaves this task: its source scene already arrives, its own text carries the gist, and no behavior has been shown to fail without the content; it is decided where open loops are dealt with, against a scenario that needs it.
  - User approval: decided under the standing instruction and logged for presentation.
  - Record proposed: unchanged.
- 2026-09-21 Decision: a memory carries the union of its source scenes' keys, and Task_1 is reviewed and up as a pull request.
  - Trigger / new insight: the Tier A review of Task_2 found that under "the keys they share" a belief learned in one place and corrected in another came back at neither, and anything consolidated across places belonged nowhere. Sharing was inherited from when scope limited what could be said; since ADR-D-0038 a key only adds a road.
