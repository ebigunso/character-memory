...
## Scope / Non-goals
- Scope: `src/**`, `tests/**`, `README.md`, `docs/design/database/**`, one decision record (the derivation rules from scene to scope key, which ADR-D-0024 leaves uncovered), this plan.
...
- validation:
  ...
  - kind: review
    required: true
    owner: reviewer
    detail: "Tier D diff review; Tier A review of the route against ADR-D-0022, D-0029 and D-0038"
...
- validation:
  ...
  - kind: manual
    required: true
    owner: user
    detail: "The decider accepts or returns the proposed record, and rules on the cue kind of a custom value, in a batch at the slice boundary"
...
- depends_on: [Task_3]
- description: |
  For each notion the scene resolves as a participant, the latest earlier experience that notion took part in, by recorded scene time and never by creation time, is brought with their state (the last interaction ADR-D-0022's validation names), and the result reports the time between it and the scene's reference time. Never met is reported as never met. A participant given only by description is not reported. Forgotten experiences do not count. An experience written by `remember` and a caller-built one count alike. Found through the notion's links and recorded times, not by scanning every episode.
- acceptance:
