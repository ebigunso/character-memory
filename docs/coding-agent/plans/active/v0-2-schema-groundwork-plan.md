...
- The write planner generates an About link from a belief to each notion it is about, and a Supersedes link for each memory it supersedes. Needed because: bounded expansion follows explicit links, not the ids a memory lists, so without them a belief reached by content could not lead to its notion, and a superseded belief would still read as current.
- A marker on interpreted memory that its grounding was given by the application, accepted in place of source experiences. Needed because: the source floor otherwise rejects a name given before any experience exists, and a source-less memory with no marker is indistinguishable from a defect (ADR-D-0028).
...
- Currency, chosen: delete the stored flag and read the chain (retention allows it, and no incoming Supersedes link from an interpreted memory). Alternative: keep the flag as a cache. That leaves two sources of truth and forces the validated write path to mutate predecessors, which is the stale-overwrite family the census found; the stats projection keeps its own derived counters, which is the right home for a cache.
...
- depends_on: [Task_2]
- description: |
  The entity keeps its id, type tag, creation time and schema version; name, aliases, entity type (the enum goes), canonical key, summary and the updated time go. The entity has no embedding surface and leaves the default candidate object types. An interpreted memory may carry assertions (subject notion, predicate, literal or notion) with a closed predicate vocabulary of one, known as; the graph stores a normalized literal (case-folded, whitespace-collapsed, Unicode-normalized) beside the name as given, and a crate-internal graph query returns the notions currently known by an exact name, several when the name is shared. An interpreted memory may be marked as given by the application, which satisfies the source floor in place of source experiences. The write planner generates the About and Supersedes links the Planner-added requirements name. Propose an ADR if the admission test passes (`durable-docs-authoring`); the decider accepts it.
- acceptance:
  ...
  - A memory with neither sources nor the given marker is still rejected; an assertion whose subject is not among the memory's subjects is rejected; an unknown predicate is rejected.
...
- validation:
  ...
  - kind: manual
    required: true
    owner: user
    detail: "The decider accepts or returns the proposed implementation record"
...
- 2026-09-20 Decision: the planning decisions, ruled by the decider before drafting.
  ...
  - Record proposed: one implementation record in Task_3 (belief form, assertions, the name query, the given marker, no entity surface); acceptance pending.
...
- Edge cases: two beliefs naming one notion at once (a nickname and a given name) are both current, since neither supersedes the other; normalization must not merge names that differ only by script in ways a person would not (keep it to case, whitespace and Unicode normalization).
