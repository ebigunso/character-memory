...
- acceptance:
  ...
  - A write with only a time is valid; a participant given in words round-trips as given and yields no entity and no link, and one given by key together with words keeps both, the link from the key and the words as recorded; an activity on a write is rejected.
