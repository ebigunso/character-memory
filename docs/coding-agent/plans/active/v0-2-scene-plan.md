...
## Goal
- A write records the scene it happened in, and a retrieval is asked from a scene: when, who, where, and optionally a topic (what the character is doing arrives with its cue in the routes plan). Every admitted memory comes back with its scene as recorded, and the result says what scene it was given and which parts were not given. Nothing is withheld by the scene (ADR-D-0038). Durable writes take one turn at a time, so the derivation this slice adds at commit cannot interleave.
