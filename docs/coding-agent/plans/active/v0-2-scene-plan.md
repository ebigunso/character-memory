...
- A retrieval gives a scene and an optional topic. Only the time is required and defaults to now. A participant given by key cues its notion; by name, every notion currently known by that name; by description, whatever memory holds whose text matches in content. An ambiguous name activates each notion it could mean and an unknown one activates nothing, and the result says which, whether or not a trace is requested; a description is reported as taken for a content cue with what it reached, with no claim that it was known or unknown. With no topic, no topic is embedded. The setting key and the custom values of a retrieval's scene are reported back and cue nothing in this slice: their reader, scope keys, arrives with the routes plan.
