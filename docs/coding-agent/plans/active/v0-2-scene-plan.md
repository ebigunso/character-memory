...
- Edge cases: a name shared by the self and another notion; a participant key for a notion that exists but has no current name; a retrieval's activity id naming a thread that was later closed.
