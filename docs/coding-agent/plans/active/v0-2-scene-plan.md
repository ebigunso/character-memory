...
- For the decider, before Task_2 and not blocking Task_1: the value audit of this draft argues that rejecting participant words on a write costs behavior. A deployment that hears voices it cannot identify can cue recall by description but cannot record that someone was there, so its memories report no participants. The smaller honest form mirrors setting words: accept a participant given in words, store and report it as given, create no notion, no link and no cue. The ruling of 2026-09-21 keeps names and descriptions of people on writes with v0.3, so this is the decider's to change.
- Otherwise none. The rulings of 2026-09-21 cover this slice; plan approval is waived for plans inside them, and this plan is reviewed (Tier D and Tier A) before dispatch.
