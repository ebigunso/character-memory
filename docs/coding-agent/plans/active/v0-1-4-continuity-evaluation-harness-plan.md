...
- 2026-07-12 03:30 Task_6 (residual scope) completed and CLOSED (reviewer APPROVED after one bounce).
  - Summary: continuity dataset-kind config validation and persistent-store reconstruct landed on CME branch feature/continuity-config-persistence (5b53fd5 initial, 58fed6f bounce fix, aec3483 lesson). Continuity configs hard-reject non-deterministic providers AND independently require both durable paths (Oxigraph persistence, sqlite stats); public reconstruct validates config before any store I/O; live restart evidence asserts all six exact identity mappings, distinct episode/observation retrieval, and suppressed-correction survival across reconstruct.
  - Validation evidence: worker and reviewer green on pinned gates; reviewer live-verified the strengthened restart assertions in an isolated worktree (one environmental cleanup failure after local Qdrant vanished, ruled non-finding; worker isolated-Qdrant live run passed cleanly).
  - Notes: bounce findings were two HIGH (non-durable configs admitted; reconstruct bypassing generic validation) + one MEDIUM (count-only restart evidence) — all closed with line-level evidence. Worker lesson recorded: restart closure matrices must cover every required store, validation-before-I/O, identity category, and behavioral state. Task_8 fix cycle still in flight (CRLF portability + parser contract enforcement).

- 2026-07-12 03:35 Task_8 completed and CLOSED (reviewer APPROVED after one bounce) — Wave 3 complete.
  - Summary: cmem-eval-continuity crate landed on CME branch feature/continuity-fixtures (762ba4e initial, 637e316 bounce fix, fc0ef90 lesson): serde schema v1 (role-free entities, caller-supplied deterministic MemoryIds/timestamps, six event variants incl. restart), seeded generator + CLI, checked-in canonical fixture with all eight scenario patterns (hubs across 3 entity kinds at degree 6, >=180-day gaps, correction chains, cross-store restart). Bounce closed: scoped eol=lf .gitattributes + renormalization makes fixture bytes checkout-portable (independently verified via fresh materialization under autocrlf=true: 29,458 bytes, 0 CRLF, blob identity to index); public parser now enforces the full lifecycle/relevance contract in event order (suite 7->14 with per-class malformed regressions); sampled-negative label semantics documented (pollution-metric contract for Task_10 unambiguous).
  - Validation evidence: reviewer gates green on pinned Windows toolchain incl. Qdrant-down 14/14 with two-process byte identity; worker full workspace gates accepted as corroboration.
  - Notes: Wave 3 branches are disjoint and both approved: feature/continuity-config-persistence (aec3483) and feature/continuity-fixtures (fc0ef90); merge-to-main decision stays with the user; Task_9 proceeds on a consolidation branch merging both. Worker lessons recorded on both bounces (restart closure matrix; fixture checkout-normalization + parser-mutation closure).

- 2026-07-12 05:30 Task_9 completed and CLOSED (reviewer APPROVED after one bounce) — Wave 4 complete.
  - Summary: continuity driver + run continuity subcommand landed on CME branch feature/continuity-driver (merge 3ccdb64 of both Wave 3 heads; 93e9b86/e17a0ac task commits; 7cf812e bounce fix; 686292e/70fee6a lessons). Driver executes fixture events only across all eight facade operations incl. real restart via the Task_6 reconstruct path; ContinuitySpec wires validate_for_dataset_kind(Continuity) as the production caller (Task_6 sequencing gap closed); per-query JSONL traces capture pack/rationales/telemetry for Task_10; no driver wall-clock/random reads; strict embedding-surface discipline. Bounce (HIGH) closed: the public trace reader now propagates decoding/I-O errors with path/line context and enforces CONTINUITY_TRACE_SCHEMA_VERSION with found/expected errors, plus corruption and wrong-version regressions.
  - Validation evidence: worker + reviewer pinned gates green (MSVC 1.97.0; GNU locally blocked by pre-existing missing dlltool, no repo change); Qdrant run-to-run reproducibility probe PASSED — worker and reviewer independently produced byte-identical result/summary/trace hashes across live cross-store-stress runs (plan risk item retired, no tolerance comparison needed).
  - Notes: rule candidate accepted for promotion on third recurrence of the public-boundary admission class (snapshot validators, fixture parser, trace reader): every new public Result-returning artifact reader must ship corrupt/invalid-encoding, partial-input, and schema-version-rejection tests before completion — to be added to CME worker rules with the Wave 5 work. Wave 5 dispatched: Task_10 (seven continuity metrics + registry) and Task_11 (restart measurement + diffable report format), sequential on feature/continuity-driver.

- 2026-07-12 10:40 Task_10 completed and CLOSED (reviewer APPROVED after two bounces; three in-flight owns-expansion rulings).
  - Summary: seven section-7 continuity metrics + registry integration landed on feature/continuity-driver (range 70fee6a..ee5b5a8: 4f9b35b worker-rule promotion, 5f5c9c4 telemetry projection/driver/fixture expansions, a263115 metrics, 465be6c + f8c8855 bounce fixes, bfe207d/ee5b5a8 lessons). Live evidence: hub decisions 9 (3 scored)/fanout 90/zero over-budget; selective decisions 9 (2 scored); registry parity 68/68/23/45 across run->summarize; committed-config two-run byte-identical reports.
  - Validation evidence: worker + reviewer pinned gates green incl. live adapter tests; reviewer independently reproduced registry parity, hand-calculated rates, live profile metrics, and committed-config reproducibility.
  - Bounce findings closed: (HIGH) summarize rebuilt the metric registry without scenarios, dropping dynamic entity-kind keys — run/summarize now share scenario-derived family with parity regression (recurrence of the CM a62f66d summarize-parity class); (MEDIUM x2, same family) lifecycle category counts double-counted decision multiplicity — every item count now deduplicates unique returned MemoryIds independently, leakage rates bounded at 1.0, decision volume reserved for lifecycle_filter_decision_count.
  - Notes: v0.1.5 tuning observation recorded in run artifacts: under CM defaults, equal-score root ordering prioritizes episode/observation over entity and default max_graph_roots=12 truncates hub entity roots; continuity runs surface their non-default max_graph_roots=max_vector_candidates regime in the config snapshot.

- 2026-07-12 15:30 Task_11 completed and CLOSED (reviewer APPROVED after one bounce) — Wave 5 complete; all [CME] implementation done.
  - Summary: restart measurement + deterministic report assembly landed on feature/continuity-driver (range ee5b5a8..f464578: b8af403/e54fdef initial, a7dc74d contract/report fixes, b860f1b Windows persist retry, f464578 lessons). Restart events probe the next scripted query before/after a real adapter drop + reconstruct; schema-versioned --report-out isolates all nondeterminism (generated_at, mutation-timestamp exclusion policy) in a metadata block; deterministic content carries per-scenario+aggregate metrics/support/coverage, full section-8 rationale samples (query + RetrievedContextPack + selectivity telemetry), fanout/selectivity decisions, stats-health events, restart observations, and the Task_10 tuning observation; assembly fails closed on count, order, and duplicate-query_id mismatches (fixture now validates query_id uniqueness globally and per scenario).
  - Validation evidence: worker and reviewer independently ran the FULL 8-scenario committed-config live suite at CM sibling 745c939 — identical operation counts and byte-identical hashes across all four artifacts, two runs each; restart restored 10 identities, recall 1.0->1.0, every delta 0; full pinned gates green both sides (reviewer with Qdrant live).
  - Bounce findings closed: (HIGH) driver sent source-object corrections with both original refs None — live CM rejects; now retains the exact continuity:// raw ref + source_conversation_id (mock/live contract-gap class); (MEDIUM) section-8 sample completeness; (MEDIUM) per-scenario registry coverage; (sharpened) query_id-based trace/row correspondence was not unique — closed with uniqueness validation + ordered id/type/text cross-checks + swapped-order/duplicate-query regressions.
  - Evidence-integrity incident recorded: worker's initial live hashes were cross-store-stress-only but reported without scenario scope; withdrawn on challenge, lesson committed (f464578) — every live-evidence claim must state scenario scope, config identity, and CM sibling provenance at the point of claim.

- 2026-07-13 04:50 Task_12 and Task_13 completed and CLOSED — Wave 6 complete.
...
- 2026-07-12 Decision: Task_11 owns expanded once in flight (orchestrator-approved): bounded retry in cmem-eval-adapter-cmem ExternalIdRegistry persist for exactly ErrorKind::PermissionDenied (4 attempts, deterministic 25/50/75ms backoff, staged NamedTempFile retained so atomic all-or-nothing replacement holds; non-PermissionDenied and final-attempt errors return immediately with RAII cleanup). Rationale: full-suite live validation was blocked by transient Windows atomic-replace contention (AV/indexer holding the destination during MoveFileExW), reproduced across three different scenarios incl. unsandboxed; retry is environmental hardening, not contract weakening — reviewer re-verified the Task_6 crash-safety contract at approval.

- 2026-07-12 Decision: Task_10 owns expanded four times during execution (all eval-side, orchestrator-approved in flight, no CM changes): (1) cmem-eval-adapter-cmem projects CM public telemetry (fanout_utilization, selectivity_decisions, typed rationale categories) into backend-neutral core structs — core stays CM-type-free, absent fields project to None (null-never-false-zero at the projection layer); (2) runner pipeline.rs fills the previously-empty continuity_driver MetricFamily; (3) driver.rs translates fixture-declared entity_external_ids into deterministic episode->entity mentions AND entity->episode involves links (ADR-I-0013 intact — declared data only; involves added because CM default selectivity specs cover involves->episode, not mentions); (4) generator.rs + checked fixture assign entity labels to concepts (first-referencing-Remember rule + background concept, total-assignment validation) and the continuity retrieval path passes caller-supplied max_graph_roots=max_vector_candidates when include_entities (surfaced in config snapshot). Rationale: Task_10 is the first end-to-end consumer of the telemetry pipeline; each expansion closed a gap between components reviewed in isolation and values actually flowing. Alternative (accept null distributions) rejected: acceptance requires real v0.1.5 tuning values. User approval: within approved plan direction; no CM-side scope touched.
