...
## Goal
- A write records the scene it happened in, and a retrieval is asked from a scene: when, who, where, and optionally a topic (what the character is doing arrives with its cue in the routes plan). Every admitted memory comes back with its scene as recorded, and the result echoes the scene it was given; an unset part means not given, and no scene is ever complete. Nothing is withheld by the scene (ADR-D-0038). Durable writes take one turn at a time, so the derivation this slice adds at commit cannot interleave.
...
- Design record consulted and deviations from its acceptance: ADR-D-0020, D-0022, D-0024, D-0029, D-0034, D-0038, ADR-I-0016, I-0020, I-0022, I-0035. Scope keys (ADR-D-0024) are deferred to their reader, not dropped. ADR-D-0029's report on each reference is delivered in part: a key and a name are reported resolved, ambiguous or unknown here, while a description is reported only as taken for a content cue, because calling a description unknown needs the admission floor that the routes plan measures; claiming it earlier would be a guess. ADR-I-0035 is delivered in part: writes are serialized and no turn holds a model call, while its invariant that recall never observes a write in progress stays with v0.3; recall can observe a write in progress today and still can after this slice.
...
- depends_on: [Task_2]
- description: |
  `RetrievalContext` takes a scene and an optional topic; `query_text` and `current_context` become the topic (a caller that used both composes them itself). The time is required and defaults to now. References are resolved inside recall: a key is an entity root; a name goes through the exact-name query and may yield several notions (ambiguous) or none (unknown); a description is a content cue. Resolved notions are entity roots for the existing expansion, which gives entity selectivity its production input back. A key or a name enters as an entity root under the existing expansion guardrails and is never presented as a vector hit. Where the existing selectivity and ranking need a cue score for a root, an explicit reference counts as a full-strength cue: it was given, not guessed, so it takes the top of the existing scale and no new number is introduced; the trace marks the root as named by the scene. A key that names no notion is reported unknown and activates nothing. Each distinct name or description in a scene is resolved once, however often it is repeated. When a topic and one or more descriptions are given, their hits merge into the one candidate list the pipeline already has, an object keeping its best score, under the existing candidate budget; roots the scene names explicitly (keys and resolved names) are entered before roots that come from content hits, under the existing root budget, in the order the scene gives its participants (an application lists who matters first), each notion once, several notions under one name in id order; the trace reports any root the budget dropped, so the same scene always admits the same roots. With no topic the topic search is skipped, not run on empty text; a description is still embedded as its own cue; with neither and no resolvable participant the pack may be empty, since the time and state routes are later. The setting key and custom values are reported back and cue nothing yet. Each admitted memory reports its scene as the Definition of Done states (every source's scene; none, explicitly, where no experience is recorded), in the result itself; the result also echoes the scene as given (no list of absences, no completeness flag) and carries how each reference resolved; the trace keeps only mechanism. No option omits by scene. Propose an implementation record for the scene's retrieval-level shape if the admission test passes.
- acceptance:
  ...
  - With no topic, an embedder that fails on the absent topic is never called for it, while a description is embedded; an interpreted memory resting on two episodes, one of them not admitted, reports both scenes; an admitted observation reports its episode's scene, and an interpreted memory that cites only observations reports the scenes of their episodes; a correction's replacement reports the scenes of its sources; a thread and an application-given belief report no scene.
  - A scene with only a time is accepted and echoed as given; nothing is omitted because of any part of the scene.
