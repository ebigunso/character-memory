...
- acceptance:
  ...
  - An explicit awaitable close releases the embedded shard before directory deletion or reopen, remains idempotent at the internal port, and leaves per-write durability and signal-only drop unchanged.
...
- acceptance:
  ...
  - The public consuming close is documented and exercised by immediate local-directory removal and fresh reopen, while the test fixture continues using the same awaitable embedded close.
  - All five checklist rows are dispositioned in the Progress Log: produced evidence for the rows this repository owns, an explicit pending-there note for the rows the evaluation repository owns.
...
- 2026-09-03 Plan approved for implementation by the decider (status approved in the first implementation change); the wave PRs are stacked on the planning PR and the stack merges in one go at phase end, so the 2026-09-02 note that plan approval was pending is closed by this entry.
...
- 2026-09-10 Decider finalization: the embedded default stands. The evaluation repository's cross-mode run (its Task_8 step 1) found no difference between embedded and service mode on the continuity suite, so ADR-I-0023's revisit trigger did not fire; the two library fixes it surfaced are on the stack (Decision Log, 2026-09-06). The stack is merged as one unit on this ruling.
...
- 2026-09-06 — Two library defects found by the evaluation repository's pre-merge cross-mode run (decider ruling 2026-09-04) and fixed on the stack before the merge.
  - Path canonicalization: the embedded adapter passed a configured store path containing parent-directory segments to the engine uncanonicalized and shard open failed; the adapter now canonicalizes the created root before deriving the shard path (regressions for parent segments and Windows paths over 260 characters).
  - Explicit release: a consumer that dropped the facade and deleted the store directory raced the owner thread, because the facade's drop is signal-only; the library gains a public consuming `CharacterMemory::close` that awaits the embedded store's release, and the port a default no-op close. Durability still never depends on close and the drop stays signal-only; ADR-I-0027 records the revision, the phase document and roadmap name the close as the one other public facade addition. This supersedes the 2026-09-02 ruling's wording "no port or facade method is added" above.
  - Evidence: continuity 23 queries and LongMemEval-S 100 questions identical across modes; LoCoMo 304 questions differ below the same-mode floor; details in the evaluation repository's plan and its pull request for Task_8 step 1.
