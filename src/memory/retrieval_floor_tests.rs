...
async fn configured_root_floors_are_reserved_before_spare_slots_are_shared() {
    ...
    memory
        ...
        .await
        .unwrap();
    for configured in [None, Some((6, 5, 1)), Some((8, 6, 2)), Some((4, 3, 1))] {
        let mut context = RetrievalContext::new("topic")
            .with_activity(ActivityRef::Thread(MemoryId::from_u128(5000)))
            .with_scene(occasion())
            .with_trace();
        context.graph_limits.max_depth = 1;
        if let Some((cap, _, _)) = configured {
        ...
        if let Some((cap, activity_count, topic_count)) = configured {
            let expanded = result
                .trace
                .unwrap()
                .graph_expansions
            ...
            assert!(result.pack.derived_memories.len() >= 3);
        }
    }
    memory.close().await.unwrap();
}
