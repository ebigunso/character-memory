...
use qdrant_client::{
    Qdrant,
    qdrant::{
        CreateCollectionBuilder, DeletePointsBuilder, Distance, PointStruct, SearchPoints,
        SearchPointsBuilder, ScoredPoint, UpsertPointsBuilder, VectorParams, VectorsConfig, PointsSelector,
    },
};
...
impl VectorDatabase for QdrantDatabaseImpl {
    ...
    async fn upsert_points(
        ...
        let upsert_req = UpsertPointsBuilder::new(collection_name, points).build();
    ...
    async fn delete_points(&self, collection_name: &str, selector: &PointsSelector)
        -> Result<(), CustomError>
    {
        let points_selector = selector
            ...
            .ok_or_else(|| {
                CustomError::DatabaseError(
                    "points_selector_one_of must be specified for delete operation".to_string(),
                )
            })?;
